    ...
    private void handleBusiness( WritableWorkbook workbook ) throws Exception {
        ...
        for( int i = 0 ; i<merIdList.size();i++ ) {
            ...
            for(int r = 0; r<merPayOrder.size(); r++){
                ...
                //  "订单金额",
                col++;
                Number orderAmountLabel = null;
                if( r%2 ==0 )
                    orderAmountLabel = new Number(col,r+1,payOrder.getAmount().doubleValue(),titleFormate02(sheet,r+1,col));
                else
                    orderAmountLabel = new Number(col,r+1,payOrder.getAmount().doubleValue(),titleFormate03(sheet,r+1,col));
                sheet.addCell(orderAmountLabel);
                // "实际支付金额",
                col++;
                Number realAmountLabel = null;
                if( r%2 ==0 )
                    realAmountLabel = new Number(col,r+1,payOrder.getRealAmount().doubleValue(),titleFormate02(sheet,r+1,col));
                else
                    realAmountLabel = new Number(col,r+1,payOrder.getRealAmount().doubleValue(),titleFormate03(sheet,r+1,col));
                sheet.addCell(realAmountLabel);
                //  "平台手续费收入",
                col++;
                Number  playformFeeLabel = null;
                if( r%2 ==0 )
                    playformFeeLabel = new Number(col,r+1,payOrder.getFee().doubleValue(),titleFormate02(sheet,r+1,col));
                else
                    playformFeeLabel = new Number(col,r+1,payOrder.getFee().doubleValue(),titleFormate03(sheet,r+1,col));
                sheet.addCell(playformFeeLabel);
                //  "平台毛利润",
                col++;
                Number  incomeLabel = null;
                if( r%2 ==0 )
                    incomeLabel = new Number(col,r+1,payOrder.getIncome().doubleValue(),titleFormate02(sheet,r+1,col));
                else
                    incomeLabel = new Number(col,r+1,payOrder.getIncome().doubleValue(),titleFormate03(sheet,r+1,col));
                sheet.addCell(incomeLabel);
                //  "通道成本",
                col++;
                Number channelFeeLabel = null;
                if( r%2 ==0 )
                    channelFeeLabel = new Number(col,r+1,payOrder.getChannelFee().doubleValue(),titleFormate02(sheet,r+1,col));
                else
                    channelFeeLabel = new Number(col,r+1,payOrder.getChannelFee().doubleValue(),titleFormate03(sheet,r+1,col));
                ...
                //   "通道费率",
                col++;
                Number channelRateLabel = null;
                if( r%2 ==0 )
                    channelRateLabel = new Number(col,r+1,channelInfo.getChannelRateFee().doubleValue(),titleFormate02(sheet,r+1,col));
                else
                    channelRateLabel = new Number(col,r+1,channelInfo.getChannelRateFee().doubleValue(),titleFormate03(sheet,r+1,col));
                ...
                //    "子商户成本"
                col++;
                Number terMerFeeLabel = null;
                if( r%2 ==0 )
                    terMerFeeLabel = new Number(col,r+1,payOrder.getTerminalFee().doubleValue(),titleFormate02(sheet,r+1,col));
                else
                    terMerFeeLabel = new Number(col,r+1,payOrder.getTerminalFee().doubleValue(),titleFormate03(sheet,r+1,col));
